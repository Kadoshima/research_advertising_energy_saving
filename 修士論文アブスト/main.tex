...
\section{まとめ}
HAR不確実度に基づく2値アドバタイジング間隔制御が，固定間隔の中間的電力でQoS制約$\Pout(\tau)\le\epsout$を満たす運用点になり得ることを実機で示した．さらに，UとCCSのアブレーションにより，Uは短間隔滞在量（電力），CCSは同電力でのQoS改善を担うことを示唆した．今後は，制約下でアドバタイジング間隔を選択する多腕バンディットへ展開する．
