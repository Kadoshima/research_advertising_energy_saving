...
\begin{figure*}[t]
  ...
  \caption{固定・方策・アブレーション（S4, scan90/70）のpower--$\Pout(\SI{1}{\second})$俯瞰．scan70では固定$\SI{500}{\milli\second}$が制約（$\Pout(\SI{1}{\second})\le 0.1$）を満たさない一方，方策は制約を満たし固定$\SI{100}{\milli\second}$より低電力となる．矢印はU/CCSの寄与を示す．}
