...
\par
固定間隔を短くすると平均消費が増え，長くすると発見遅延が増えるため，単純な固定設定では省電力とQoSの両立が難しい．そこで本研究では，QoS制約を$\Pout(\tau)\le\epsout$（本稿では$\tau=\SI{1}{\second}$，$\epsout=0.1$）で表し，これを満たしつつ送信イベント当たり電荷$q_{\mathrm{event}}$（\si{\micro\coulomb}/event）を小さくする設計問題として扱う．
...
\begin{figure}[t]
  \centering
  \begin{tikzpicture}[font=\scriptsize,>=Latex,block/.style={draw,rounded corners,align=center,minimum width=16mm,minimum height=7mm}]
    \node[block] (tx) {TX\\(DUT)};
    \node[block,left=4mm of tx] (rx) {RX\\(受信ログ)};
    \node[block,right=4mm of tx] (txsd) {TXSD\\(電力ログ)};
    \node[block,above=5mm of tx,minimum width=22mm] (ina) {INA219\\(電流/電圧)};
    \node[block,above=3mm of ina,minimum width=22mm] (ps) {外部電源\\(3.3V)};

    \draw[->] (ps) -- node[right]{電源} (ina);
    \draw[->] (ina) -- node[right]{電源} (tx);
    \draw[->,thick] (tx) -- node[above]{BLE広告} (rx);
    \draw[->] (tx) -- node[above]{UART} (txsd);
    \draw[->] (txsd) -- node[right]{I\textsuperscript{2}C} (ina);
    \draw[->,dashed] (tx) to[bend left=18] node[below]{SYNC} (rx);
    \draw[->,dashed] (tx) to[bend left=18] node[below]{SYNC/TICK} (txsd);
  \end{tikzpicture}
  \caption{計測系の信号関係（簡易）}
...
\begin{figure*}[t]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_role_separation_overview}
  \caption{固定条件・方策・アブレーションの全体俯瞰（power--$\Pout(\SI{1}{\second})$）．受信条件悪化（scan70）では固定$\SI{500}{\milli\second}$が$\epsout$を超え得る一方，方策は$\epsout$を満たしつつ固定$\SI{100}{\milli\second}$より低電力となる．矢印はUおよびCCSの寄与（Uは短間隔滞在量，CCSは同電力でのQoS改善）を示す．}
...
\section{まとめ}
HAR不確実度に基づく2値広告間隔制御が，固定間隔の中間的電力でQoS制約$\Pout(\tau)\le\epsout$を満たす運用点になり得ることを実機で示した．さらに，UとCCSのアブレーションにより，Uは短間隔滞在量（電力），CCSは同電力でのQoS改善を担うことを示唆した．今後は，広告間隔候補の多段化と，制約下で広告間隔を選択するSafe Contextual Banditへ展開する\scite{lattimore2020bandit}．
