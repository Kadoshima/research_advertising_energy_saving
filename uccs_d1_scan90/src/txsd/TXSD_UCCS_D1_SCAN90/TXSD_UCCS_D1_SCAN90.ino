// TXSD_UCCS_D1_SCAN90.ino (uccs_d1_scan90)
// INA219 logger for Step D1. Start/stop via SYNC (TX GPIO25 -> TXSD GPIO26).
...
static bool condInfo(uint8_t id, uint16_t* interval_ms, const char** tag){
  switch(id){
    case 1: *interval_ms = 100; *tag = "fixed100"; return true;
    case 2: *interval_ms = 500; *tag = "fixed500"; return true;
    case 3: *interval_ms = 0;   *tag = "policy";   return true;
    default: break;
  }
  *interval_ms = 0;
  *tag = "unk";
...
static inline String nextPath(uint8_t id){
  ...
  const char* tag="unk";
  (void)condInfo(id, &interval_ms, &tag);
...
static void startTrial(uint8_t id){
  ...
  uint16_t interval_ms=0;
  const char* tag="unk";
  (void)condInfo(condId, &interval_ms, &tag);
  f.printf("# meta, firmware=TXSD_UCCS_D1_SCAN90, cond_id=%u, interval_ms=%u, tag=%s, subject=%s\r\n",
           (unsigned)condId, (unsigned)interval_ms, tag, SUBJECT_ID);
  if (condId == 3){
    f.printf("# meta, policy=actions{100,500} u_mid=0.20 u_high=0.35 c_mid=0.20 c_high=0.35 hyst=0.02\r\n");
  }
  f.flush();
...
static void endTrial(){
  ...
  f.printf("# summary, ms_total=%lu, E_total_mJ=%.3f, subject=%s\r\n",
           (unsigned long)ms_total, E_mJ, SUBJECT_ID);
...
void setup(){
  Debug.begin(115200);
  Debug.println("[PWR] FW=TXSD_UCCS_D1_SCAN90");
