...
まず，オフライン評価の比較表について述べる．
\tabref{tab:policy_table}に，固定とルールベース方策の比較表（例）を示す．表中の各行は方策（固定間隔およびルールベース），各列は滞在比率（share100/500/1000/2000），PDR\_unique，期限超過率$P_{\mathrm{out}}(1\,\mathrm{s})$，平均遅延$TL_{\mathrm{mean}}$，イベントあたりエネルギー（E/adv），および平均電力（P）を表す．

\begin{table}[tb]
  \centering
  \caption{方策の比較表（各方策の滞在比率・PDR・Pout・TL・エネルギー・電力，mHealth合成）}
  ...
  \begin{tabular}{lrrrrrrrrr}
    \toprule
    方策 & share100 & share500 & share1000 & share2000 & PDR\_unique & $P_{\mathrm{out}}(1\,\mathrm{s})$ & $TL_{\mathrm{mean}}$[s] & E/adv[μJ] & P[mW] \\
    \midrule
