...
\chapter{オフライン評価：mHealth合成とルールベース方策探索}

\section{目的}
実機実験の探索空間を縮小し，説明可能な代表方策を選定するために，オフライン評価を行う．本章では，mHealth由来の時系列（不確実度・安定度・複合スコア）に対してルールベース方策を適用し，固定間隔の電力テーブルと組み合わせて，QoS制約下の省電力運用点を探索する．

\section{入力データ（mHealth合成）}
...
\subsection{QoSの推定}
オフライン評価におけるQoS推定は，固定間隔の実測から得られる$P_{\mathrm{out}}(\tau\mid a)$等の指標を参照し，方策の滞在比率に基づき合成する（詳細は付録A）．動的切替時の非定常性（切替直後の遅延分布など）は近似に含まれないため，実機評価で差分を検証する必要がある．

\section{Paretoフロントとδ帯プロット}
...
\section{行動空間の縮退（\{100,500\}）}
実機の動的制御を最小構成で成立させるため，行動空間を\{100,500\}に限定して代表方策を再選定する．この縮退により，実機側の実装・解析の複雑性を抑えつつ，電力低下の主効果（短間隔滞在の削減）に整合した比較が可能となる．

\section{選定方策の例}
オフライン評価の出力は，単一の最良方策ではなく，「制約$\delta$付近で成立する候補集合」として整理する．その上で，実機実験の本数制約や説明容易性を踏まえて代表点を選定する．代表点の一覧は付録Cに示す．

\section{まとめ}
本章では，mHealth合成と固定間隔の実測電力テーブルを組み合わせたオフライン評価により，QoS制約下での省電力運用点を探索する枠組みを示した．次章では，計測系の健全化と再現性確保の観点から，ログ設計と実験手順を整理する．
