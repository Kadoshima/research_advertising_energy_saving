...
\subsection{目的}
実機実験の探索空間を縮小し，説明可能な代表方策を選定するために，オフライン評価を行う．本節では，mHealth由来の時系列（不確実度・安定度・複合スコア）に対してルールベース方策を適用し，固定間隔の電力テーブルと組み合わせて，QoS制約下の省電力運用点を探索する．オフライン評価の主軸は平均電力$\overline{P}$であり，一次KPIである$q_{\mathrm{event}}$の検証は実機評価で行う．

\subsection{入力データ（mHealth合成）}
オフライン評価では，実機上でのHAR推論を直接用いる代わりに，既存データセット（UCI mHealth）由来の時系列から$U,S,\mathrm{CCS}$を合成し，方策を適用する\scite{uci_ml_repo}．これにより，実機で高価な全探索を行う前に，閾値やヒステリシスの候補を絞り込める．
...
\begin{figure}[tb]
  ...
  \caption{制約帯プロット（オフライン評価, $\epsilon=0.1$）}
