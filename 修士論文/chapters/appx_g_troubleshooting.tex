...
\begin{longtable}{p{0.28\linewidth}p{0.32\linewidth}p{0.34\linewidth}}
  ...
  TL/Poutが不自然に小さい & RXと真値の時間軸ずれ & 定数オフセット補正を適用したか \\
...
\subsection{代表的トラブル}
\noindent アドバタイジングOFFの方が消費が大きい（$\Delta E<0$）について述べる．

アドバタイジングOFFがアドバタイジングONより大きい結果は，物理的に矛盾して見えるため，最優先で切り分ける必要がある．本研究では，無線スタックの影響を議論する前に，計測・解析起因の破綻を疑う．
\begin{itemize}
  \item 主要因候補：I/Oボトルネックによる欠損，単位換算ミス，条件混在（コード系列の違い）．
  \item 対策：生ログ（mV/μA）から再積分し，summaryと一致することを監査する．比較は同一コード系列で統一し，sleepの有無も条件として明示する．
  \item 判断：監査を通過した上で$\Delta E<0$が再現する場合，電源ドメインや無線状態遷移など物理要因を再検討する．
\end{itemize}
\noindent TL/Poutの過小評価（開始時刻のずれ）について述べる．

RXログの開始が真値より遅れる等の理由で，RXの時間軸と真値の時間軸が一致しない場合がある．このずれを補正しないと，TLや$P_{\mathrm{out}}(\tau)$が過小評価される．本研究では，seqから定数オフセットを推定して補正する手順を採用する（\secref{sec:stress_fixed_metrics}）．
\noindent UARTデータ化け（数値列への文字混入）について述べる．

UARTログが壊れると，欠損や桁ズレが連鎖し，積分と分母（試行時間・アドバタイジング回数）が不整合になる．
\begin{itemize}
...
\end{itemize}
\noindent SD I/O失敗（open/init）について述べる．
