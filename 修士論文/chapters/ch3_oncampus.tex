...
本研究の計測は，汎用マイコン（ESP32系）と電流センサ（INA219）を中心に構成する．\tabref{tab:hw}に，主要な構成要素と役割を示す．
...
アドバタイジングイベント数$N_{\mathrm{adv}}$（定義は\secref{sec:metrics_detail}）は，到達性指標の分母として重要である．可能であれば，TXがTICK信号を出力し，TXSD側で割り込みによりアドバタイジング回数を物理カウントする．TICKが利用できない場合は，試行時間とアドバタイジング間隔から期待値を算出するが，動的制御下では推定誤差が生じる可能性がある点に留意が必要である．
\noindent 配線例（UCCS評価）について述べる．
本研究で用いた配線例を\tabref{tab:wiring_example}に示す（実験ディレクトリのREADMEに準拠）．ピン番号は実装に依存するため，運用時はRunbookに記録して固定する（\secref{sec:runbook}）．
...
TXSDログは，時刻と計測値（電圧・電流）を行ごとに保持し，末尾に集約値を出力する．積分は$\Delta t$を用いた逐次積分とし，後処理でも再積分して整合性を確認する（計測系の健全化は\secref{sec:measurement_system}で述べる）．
...
省電力指標$q_{\mathrm{event}}$は，電荷積分に基づくため，ログ欠損や単位不整合の影響を強く受ける．そのため，アドバタイジングON/OFF差分による基準補正や，生ログを用いた再積分による整合性確認が不可欠である．また，送信側がlight sleep等に入れる条件では，アドバタイジング間隔を伸ばすことが平均電力低下に直結する．sleepの有無は条件として明示し，同一コード系列で比較する（運用手順は\secref{sec:runbook}）．
...
本研究の解析は，ログを読み込んで指標を算出し，条件ごとに集約して図表を生成する流れである．最小構成として，以下を満たすことを解析の妥当性判断基準とする．
\begin{itemize}
...
\subsection{まとめ}
本節では，実験装置の構成，同期設計，ログ設計，および運用上の前提を整理した．以降の節では，計測系の不具合要因と修正履歴を\secref{sec:measurement_system}にまとめ，評価の前提を固定する．
