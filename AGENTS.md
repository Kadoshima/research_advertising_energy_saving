...
- データの捏造作成は禁止（実測データを勝手に生成しない）
- 定数閾値パラメータの勝手な設定は禁止（根拠なく値を決めない）
...
- 失敗中断した試行は削除せず mode=ABORT や status=failed でマークする
...
- TX/RX/TXSDのログは同期方法を明記する（SYNC信号、tl_time_offset_ms、seqinterval_ms等）
