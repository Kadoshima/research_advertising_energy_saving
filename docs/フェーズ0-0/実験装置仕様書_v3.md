...
- 信号: SYNC_START=TX GPIO26 → TXSD/RX GPIO26、SYNC_END=TX GPIO25 → TXSD/RX GPIO25。TICK_OUT=TX GPIO27 → TXSD GPIO33。
...
| ノード | 役割 | 主配線 |
|--------|------|--------|
| TX | 被測定体（BLE広告） | 3V3/GND（INA経由）, SYNC_START=26, SYNC_END=25, TICK_OUT=27, LED=2 |
| TXSD | 電力ロガー（INA直読み＋SD） | SDA=21, SCL=22, SYNC_IN=26, SYNC_END=25, TICK_IN=33, SD: CS=5/18/19/23 |
| RX | 受信ロガー（パッシブスキャン） | SYNC_IN=26, SYNC_END=25, SD: CS=5/18/19/23 |
...
- 2026-01-19: 配線更新: SYNC_OUT=TX GPIO26、SYNC_IN=RX GPIO26 / TXSD GPIO26（SYNC_ALT=25は未使用）。
- 2026-01-21: SYNC_START=TX GPIO26、SYNC_END=TX GPIO25を導入。RX/TXSDはGPIO26/25のエッジ検出で開始/終了。
- TICK配線: TX GPIO27 -> TXSD GPIO33 (RISING) を維持。
- 機材: TXボードを ESP32 WROVER DEV に交換（FQBN: esp32:esp32:esp32wroverkit）。
- 電源: TX/RX/TXSD/外部電源の共通GNDを再接続（浮き対策のため見直し）。
