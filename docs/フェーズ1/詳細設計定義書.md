...
* **C3. BLE Advertiser**：**interval適用**、3ch（37/38/39）広告、適用成否をイベント化
...
* **サブモジュール**：
  ...
  * **KPI Builder**：TL分布、Pout(τ)、PDR、平均電流、イベント電荷の集計CSVを出力
...
* **イベント境界**：GPIOで**広告イベント開始/終了**をマーキング
* **前後校正**：ゼロ/ゲイン、温度ログ
...
* 生IMUは**外部送信しない**。広告内容は**アクティビティ種別/統計メタ**に限定（フェーズ1は固定ビーコンでも可）
