// === RX_1201.ino (SYNC非依存・seqロガー) ===
// パッシブスキャン受信ロガー。SYNCを使わず、MFDのseqをそのまま記録。
// セッション全体を1ファイルに吐き出し、trial境界はオフラインでseqの巻き戻りから復元。
...
static const int SYNC_IN  = 26; // 未使用（安全のためPulldownのみ）
...
static volatile uint16_t rxHead=0; // 書き込み側
static uint16_t rxTail=0;          // 読み出し側
static uint32_t bufOverflow=0;
static uint32_t lastFlushMs=0;

// trial状態（セッション全体を1ファイルで記録）
static bool     trial=true;
static uint32_t t0Ms=0;
static uint32_t rxCount=0;
static uint32_t trialIndex=1; // 常に1本目
static File     f;
static const char FW_TAG[]="RX_1201_SEQ_LOG";
...
void setup(){
  ...
  pinMode(SYNC_IN, INPUT_PULLDOWN); // SYNCは未使用
