// Mode B logger: 待機ベースライン計測 (時間終了、TICK不要)
// 起動時に自動開始し、TRIAL_MS後に終了してDeep Sleepへ。
...
static const uint32_t TRIAL_MS  = 300000;   // 5分 (必要に応じ調整)
static const uint64_t SLEEP_US  = 0ULL;     // 記録後は無期限Deep Sleep
...
static inline String nextPath(){
  ...
  for(uint32_t id=1;;++id){
    snprintf(p,sizeof(p),"/logs/trial_%03lu_offB.csv",(unsigned long)id);
...
static void startTrial(){
  ...
  f.printf("# meta, firmware=TXSD_ModeB_1202, trial_index=1, mode=B\n");
  t0_ms = millis();
