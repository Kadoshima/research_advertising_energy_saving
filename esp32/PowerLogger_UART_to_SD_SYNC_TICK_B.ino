// === PowerLogger_UART_to_SD_SYNC_TICK_B.ino ===
// Board: ESP32 Dev Module (Arduino-ESP32 v3.x)
//
// 機能：TXM(②)からの UART 受信(v,i,p)を SD に保存。SYNCで試行開始/終了。
...
String nextPath(){
  ...
  for (uint32_t id=1;;++id){
    snprintf(p,sizeof(p),"/logs/trial_%03lu.csv",(unsigned long)id);
  ...
  while (uart1.available()){
    ...
    if (c == '\n'){
      if (logging && f){
        ...
        uint32_t dt   = tNow - tPrev;        // ← dtを使う
        tPrev = tNow;

        double v=0, i=0, p=0;
        sscanf(lineBuf.c_str(), "%lf,%lf,%lf", &v,&i,&p);

        // FIX: V×I からパワーを算出して積分
        double p_calc_mW = v * i;            // V[Volt] × I[mA] = mW
        E_mJ += p_calc_mW * (dt / 1000.0);   // ← dt_msではなく dt を使用

        // ログは従来通り v,i,p を保存（pはINA219のgetPower_mW）
