# まとめ（scan90 v5 / δ tight / power table=robust）
...
`pareto_front_v7_power_table_scan90_v5_sleep_on_n9_10/pareto_sweep.csv` から、各δで `avg_power_mW` 最小の点を選定。

| δ | avg_power_mW | pout_1s | share100 | share500 | share2000 | adv_rate | switch_rate | params(u_mid,u_high,c_mid,c_high,hyst) |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |
| 0.02 | 196.12 | 0.011 | 0.865 | 0.121 | 0.014 | 8.90 | 0.026 | (0.15,0.35,0.10,0.35,0.08) |
| 0.03 | 195.09 | 0.030 | 0.816 | 0.121 | 0.063 | 8.44 | 0.029 | (0.10,0.35,0.15,0.35,0.08) |
| 0.04 | 194.93 | 0.040 | 0.813 | 0.096 | 0.092 | 8.36 | 0.028 | (0.10,0.35,0.20,0.35,0.08) |

## 読み（結論）

- `Fixed 500ms` の `pout_1s≈0.0322` が境界になる（δ<0.0322 だと Fixed500 は不可能）。
- δ=0.03（Fixed500が落ちる帯）では、ルールは `avg_power≈195.09mW` で `Fixed100(198.56mW)` より **約−3.47mW（約−1.75%）** 低い。
